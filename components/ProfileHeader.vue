...
watch(
    ...
    (newVal) => {
        if(newVal){
            ...
            if(changedFields.value.age !== props.user.age || changedFields.value.location !== props.user.location){
    ...
    <div class="profile-header">
        ...
        <div class="profile-info">
            <div class="profile-avatar-container">
                <img :src="`/avatars/${props.user?.avatarPath}`" alt="Profile Avatar" class="profile-avatar" />
            </div>
            <div class="profile-details">
                <template v-if="props.editMode">
                    <h2>{{ props.user?.username }}</h2>
                    <span>Age:</span><input type="text" v-model="age" :placeholder="props.user?.age"/>
                    <input type="text" v-model="location" :placeholder="props.user?.location"/>
                </template>
                <template v-else>
                    <h2>{{ props.user?.username }}</h2>
